...
| :--- | :--- | :--- |
| **解析器架构** | 双引擎架构：<br>1. V1：PEG 文法文件（Packrat Parsing）；<br>2. V2：**dicescript 字节码虚拟机 (RollVM)**，含 Lexer、AST Parser、Code Generator、Stack-based VM。 | **单行正则表达式解析器**：<br>`/^(\d+)?d(\d+)?(?:(kh|kl|dh|dl)(\d+))?(?:\s*([+-])\s*(\d+))?(?:\s*(?:x|\*)\s*(\d+))?(?:\s+(.+))?$/i` |
| **复合算术运算** | **完全图灵完备/复杂四则运算**：<br>支持加减乘除、取模、乘方、位运算，括号无限嵌套，如 `(1d6+2)*3+2d20kh1`。 | **仅支持单项掷骰加减常数**：<br>如 `3d6+2`；**不支持**多组骰子相加（如 `1d20+2d6` 会解析失败），**不支持**括号嵌套。 |
| **骰子修饰符 (Keep/Drop)** | 支持 `kh` / `kl` / `dh` / `dl` / `k` / `q`；原生支持中文**“优势”**（等价 `kh1`）、**“劣势”**（等价 `kl1`）。 | 支持 `kh` / `kl` / `dh` / `dl`，通过 `applyKeepDrop` 对投掷数组升序排序后截断；**尚未直接在正则内支持中文“优势/劣势”**。 |
...
| :--- | :--- | :--- | :--- |
| **`.r` / `.rd` / `.roll`** | 1. 支持标准表达式（如 `3d6+2`、`1d20+2d6+5`）；<br>2. 支持多轮重复（`3#r 1d20`）；<br>3. 支持优劣势与极值丢弃（`2d20优势`, `4d6k3`）；<br>4. 支持原因文本（`.r 1d20 力量`）。 | 1. 支持单项掷骰（`3d6+2`）及原因标注；<br>2. 支持多轮重复（顶层正则解包循环）；<br>3. 支持 `kh/kl/dh/dl`。<br>4. 执行消耗计入 `context.budget` 预算桶。 | **DiceFunc 暂不支持复合多项式（`1d20+2d6`）及括号嵌套**；不支持中文“优势/劣势”直接解析。 |
| **`.rh` / `.rdh` (暗骰)** | 掷骰过程与结果通过私聊发送给指令发送者本人，群内仅提示“进行了暗骰”。私聊失败时提供告警。 | **暂未实现**（未注册 `.rh` 处理函数）。 | 官方 QQ 机器人体系下，群身份与 C2C 身份由于单向散列无法可信互联，为防隐私泄露首期暂缓。 |
...
| 指令 | SealDice-Core 规范与算法 | DiceFunc 当前实现与算法 | 差异与缺失分析 |
| :--- | :--- | :--- | :--- |
| **`.st` (属性管理)** | 1. 批量设置：`.st 力量70 敏捷60 hp10`；<br>2. 表达式修改：`.st hp+1d4`、`.st 理智-3`；<br>3. 代骰：`.st hp-5 @张三`；<br>4. 查看与过滤：`.st show`、`.st show 50`；<br>5. 动态联动群名片。 | 1. 批量设置与加减：`.st 力量 60 敏捷 70`、`.st HP+2`；<br>2. 清空与删除：`.st clr`、`.st rm 力量`；<br>3. 查看展示：`.st show`、`.st show 力量 敏捷`。 | **差异**：<br>1. DiceFunc 当前正则仅支持整数加减，**不支持 `.st hp+1d4` 骰点表达式加减**；<br>2. **不支持代骰 `@他人`**；<br>3. 不支持计算属性联动。 |
...
| **`.log end` / `.log stop`** | 终结记录，生成 HTML/TXT 排版文件并**自动上传至 SealDice 跑团日志公共平台**，返回分享外链。 | 将日志标记为 `closed` 状态，生成待归档事实，等待后台写入 R2 对象存储。 | SealDice 直接集成官方集中式查看平台；DiceFunc 专注于 R2 独立私有对象存储归档。 |
| **`.log stat`** | 统计日志总字数、掷骰总次数、极值出目分布（大成功/大失败榜单）、活跃玩家排行榜。 | 仅展示当前日志状态与版本序号。 | **DiceFunc 缺乏日志明细的聚合统计分析算法**。 |
| **`.log export`** | 导出文本文件或通过配置的 SMTP 发送邮件附件。 | **未实现命令行直导**（日志由 R2 归档和 Workers API 统一交付）。 | 架构取舍差异。 |
...
1. **P1 核心跑团指令补齐（高频必需）**：
   ...
   - 实现 `.en` 技能成长检定；
   - 补齐 DND5e 死亡豁免 `.ds`。
2. **P2 骰点表达式解析器重构（Pratt Parser）**：
   - 彻底废除当前单行 `DICE_REGEX` 正则；
   - 引入支持前缀/中缀运算的 **Pratt Parser**，支持复合多项式（`1d20+2d6+5`）、括号嵌套（`3*(1d6+2)`）以及中文“优势/劣势”关键词原生解析。
3. **P3 角色库与属性计算升级**：
