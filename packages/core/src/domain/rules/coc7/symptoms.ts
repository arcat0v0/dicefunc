const COC7_FEAR_SYMPTOMS_RAW: Record<number, string> = {
...
const COC7_MANIA_SYMPTOMS_RAW: Record<number, string> = {
...
export function lookupFearSymptom(roll: number): string {
  return COC7_FEAR_SYMPTOMS_RAW[roll] ?? '';
}

export function lookupManiaSymptom(roll: number): string {
  return COC7_MANIA_SYMPTOMS_RAW[roll] ?? '';
}
